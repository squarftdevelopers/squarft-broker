...
import { projectOverviewApi } from "../../services/projectApi";
...
export default function Home() {
    ...
    const brokerSummary = [
        { label: "Total Projects", value: brokerStats.total_properties ?? 0 },
        { label: "Total Sale", value: brokerStats.sales ?? 0 },
        { label: "Pending", value: brokerStats.pending ?? 0 },
        { label: "Rejected", value: brokerStats.rejected ?? 0 },
    ];
    const branchEarning = brokerStats.branch_channel_partner_earning_summary || brokerStats.branchEarningSummary || {};
    const channelPartnerCount = branchEarning.channel_partner_count ?? branchEarning.channelPartnerCount ?? 0;
    const earningAmount = branchEarning.max_monthly_earning_label ?? branchEarning.maxMonthlyEarningLabel ?? "₹0";
        ...
        >
                        ...
                        >
                            ...
                            <View className="flex-row justify-between" style={{ zIndex: 1, elevation: 1, position: "relative" }}>
                                {overviewLoading ? (
                                    <>
                                        {[0, 1, 2, 3].map((i) => (
                                            <View key={i} className={`flex-1 bg-white/15 rounded-xl overflow-hidden ${i < 3 ? "mr-1.5" : ""} min-h-[72px]`}>
                                                <View className="bg-white/20 py-2.5 items-center">
                                                    <SkeletonBox width={42} height={8} borderRadius={4} style={{ backgroundColor: "rgba(255,255,255,0.3)" }} />
                                                </View>
                                    ...
                                    brokerSummary.map((item, index) => (
                                        <View key={item.label} className={`flex-1 bg-white rounded-xl overflow-hidden shadow-sm border border-white/50 min-h-[72px] ${index < brokerSummary.length - 1 ? "mr-1.5" : ""}`}>
                                            <View className="bg-[#4A43EC] py-2.5 items-center px-1 min-h-[34px] justify-center">
                                                <Text className="text-white text-[8px] font-lato-bold uppercase text-center" numberOfLines={2}>{item.label}</Text>
                                            </View>
                                            <View className="py-3 items-center bg-white">
                                                <Text className="text-[#1A1A1A] text-[13px] font-lato-bold">{item.value}</Text>
                                            </View>
                                        </View>
                                    ))
                                )}
            ...
            {activeTab === "Overview" && (
                <View className="bg-white px-4 pt-3 pb-2">
                    <View className="overflow-hidden rounded-2xl mb-4 bg-[#3D34E5]">
                        <Image source={require("../../assets/images/banner2.png")} className="w-full h-[132px]" resizeMode="cover" />
                    </View>

                    <View className="flex-row gap-3 mb-4">
                        <TouchableOpacity
                            onPress={() => router.push("/(tabs)/addProject")}
                            className="flex-1 h-11 rounded-xl bg-[#4A43EC] items-center justify-center flex-row"
                        >
                            <Ionicons name="add-circle-outline" size={17} color="white" />
                            <Text className="ml-2 text-[12px] text-white font-lato-bold">Add Project</Text>
                        </TouchableOpacity>
                        <TouchableOpacity
                            onPress={() => router.push("/(tabs)/favourite")}
                            className="flex-1 h-11 rounded-xl border border-[#DDD9FF] bg-[#F5F3FF] items-center justify-center flex-row"
                        >
                            <Ionicons name="person-add-outline" size={17} color="#4A43EC" />
                            <Text className="ml-2 text-[12px] text-[#4A43EC] font-lato-bold">Add Client</Text>
                        </TouchableOpacity>
                    </View>
                    ...
                    <TouchableOpacity
                        ...
                        className="flex-row items-center rounded-2xl border border-[#E5E7EB] bg-white px-4 py-3.5 mb-2"
                    >
                        <Image source={require("../../assets/images/wallet.png")} style={{ width: 38, height: 38 }} resizeMode="contain" />
                        <View className="flex-1 ml-3">
                            <Text className="text-[12px] text-[#1F2937] font-lato-bold">
                                {channelPartnerCount} channel partner{Number(channelPartnerCount) === 1 ? "" : "s"} earn up to <Text className="text-[#10B981]">{earningAmount}</Text>
                            </Text>
                            <Text className="mt-1 text-[10px] text-[#94A3B8] font-lato">View commission and wallet balance</Text>
                        </View>
