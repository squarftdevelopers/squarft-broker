...
export default function OtpVerification() {
    ...
    const { otp, otpFlow, otpToken, name, mobile, loading, error } = useSelector((state) => state.auth);
    const inputs = useRef([]);
    ...
    return <KeyboardAvoidingView className="flex-1" behavior={Platform.OS === "ios" ? "padding" : "height"}>
        <TouchableWithoutFeedback onPress={Keyboard.dismiss}><View className="flex-1">
            <StatusBar style="light" />
            <View style={{ paddingTop: 64, paddingBottom: 40, paddingHorizontal: 24, backgroundColor: "#4A43EC" }}>
                <View style={{ width: 60, height: 60, overflow: "hidden", marginBottom: 1 }}><Image source={logo} style={{ width: 110, height: 110, margin: -26 }} resizeMode="contain" /></View>
            ...
            <View className="flex-1 bg-white px-8 pt-10">
                <View className="flex-row justify-center mb-10" style={{ gap: 12 }}>{otp.map((digit, index) => <TextInput key={index} ref={(ref) => { inputs.current[index] = ref; }} value={digit} onChangeText={(text) => change(text, index)} onKeyPress={(event) => event.nativeEvent.key === "Backspace" && !otp[index] && index > 0 && inputs.current[index - 1]?.focus()} keyboardType="number-pad" textContentType={index === 0 ? "oneTimeCode" : "none"} autoComplete={index === 0 ? "sms-otp" : "off"} maxLength={index === 0 ? 6 : 1} style={{ width: 48, height: 56, borderWidth: 1, borderColor: digit ? "#4A43EC" : "#E5E7EB", borderRadius: 12, textAlign: "center", fontSize: 18, color: "#000" }} />)}</View>
                {error ? <Text className="text-red-500 text-[13px] mb-4 text-center">{error}</Text> : null}
                <TouchableOpacity onPress={verify} disabled={loading} className="bg-[#4A43EC] rounded-2xl py-4 items-center mb-10">{loading ? <ActivityIndicator color="#fff" /> : <Text className="text-white text-[15px] font-semibold">Submit</Text>}</TouchableOpacity>
                <View className="flex-row justify-center"><Text className="text-gray-500 text-[14px]">Didn’t get the OTP?  </Text><TouchableOpacity onPress={resend}><Text className="text-[#4A43EC] text-[14px] font-semibold">Resend OTP</Text></TouchableOpacity></View>
            </View>
