...
import { clearError, setName, setOtpFlow, sendOtpApi } from "../../store/slices/authSlice";
...
export default function Register() {
    ...
    const mobile = useSelector((s) => s.auth?.mobile || "");
    ...
    const submit = useCallback(async () => {
        ...
        const r = await dispatch(sendOtpApi({ phone: mobile, purpose: "register" }));
        if (sendOtpApi.fulfilled.match(r)) {
            router.push("/(auth)/otp-verification");
        }
    }, [name, loading, mobile, dispatch]);
                ...
                <View className="flex-1">
                    ...
                    >
                        <View style={{ width: 60, height: 60, overflow: "hidden", marginBottom: 1 }}>
                            <Image
                                source={logo}
                                style={{ width: 110, height: 110, margin: -26 }}
                                resizeMode="contain"
                            />
                        </View>
                        <Text className="text-white text-[26px] font-manrope-bold mb-5">
                            Complete Login
                        </Text>
                        ...
                        <TouchableOpacity onPress={() => router.replace("/(auth)/login")}>
                            <Text className="text-white text-[14px] mt-3 underline">
